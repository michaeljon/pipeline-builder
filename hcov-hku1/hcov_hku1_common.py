...
def updateDictionary(script: TextIOWrapper, options: OptionsDict):
    ...
    script.write(
        """
if [[ ! -f {REFERENCE}/hcov-hku1.dict ]]; then
    java -jar {BIN}/picard.jar CreateSequenceDictionary \\
        -R {REFERENCE}/hcov-hku1.fasta \\
        -O {REFERENCE}/hcov-hku1.dict
else
    echo "Reference dictionary {REFERENCE}/hcov-hku1.dict ${{green}}already present${{reset}}"
fi
...
if [[ ! -f {REFERENCE}/ref_genome_autosomal.interval_list ]]; then
    ...
    egrep '(NC_006577.2)\\s' {REFERENCE}/hcov-hku1.fasta.fai |
        awk '{{print $1"\\t1\\t"$2"\\t+\\t"$1}}' |
        cat {REFERENCE}/hcov-hku1.dict - >{REFERENCE}/ref_genome_autosomal.interval_list
else
...
if [[ ! -f {SORTED} || ! -f {SORTED}.bai ]]; then
    java -Xmx8g -jar {BIN}/picard.jar MarkDuplicates \\
        --TAGGING_POLICY All \\
        --REFERENCE_SEQUENCE {REFERENCE}/hcov-hku1.fasta \\
        -I {UNMARKED} \\
        -O {SORTED} \\
...
if [[ ! -f {VCF} ]]; then
    ...
    bcftools mpileup \\
        ...
        --threads 4 \\
        --output-type u \\
        --fasta-ref {REFERENCE}/hcov-hku1.fasta \\
        {BAM} 2>/dev/null | \\
    bcftools call \\
...
if [[ ! -f {PIPELINE}/{SAMPLE}.annotated.vcf ]]; then
    ...
    java -jar ~/bin/snpEff/snpEff.jar -htmlStats {PIPELINE}/{SAMPLE}.snpeff.html NC_006577.2 {VCF} >{ANNOTATED}
...
def runPipeline(script: TextIOWrapper, options: OptionsDict, prefix: str):
    ...
    script.write(
        """
if [[ ! -f {REFERENCE}/hcov-hku1.flat.fasta ]]; then
    tail -n +2 {REFERENCE}/hcov-hku1.fasta | 
        grep -v '^>' | 
        tr -d '\\n' | 
        sed 's/\\(.\\)/\\1 /g' | 
        tr ' ' '\\n' > {REFERENCE}/hcov-hku1.flat.fasta
else
...
if [[ ! -d {STATS}/{SAMPLE}_bcfstats ]]; then
    bcftools stats --fasta-ref {REFERENCE}/hcov-hku1.fasta {PIPELINE}/{SAMPLE}.vcf > {STATS}/{SAMPLE}.chk
...
if [[ ! -f {STATS}/{SAMPLE}.samtools.coverage ]]; then
    samtools coverage -d 0 \\
        --reference {REFERENCE}/hcov-hku1.fasta \\
