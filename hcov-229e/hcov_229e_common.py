...
def updateDictionary(script: TextIOWrapper, options: OptionsDict):
    ...
    script.write(
        """
if [[ ! -f {REFERENCE}/hcov-229e.dict ]]; then
    java -jar {BIN}/picard.jar CreateSequenceDictionary \\
        -R {REFERENCE}/hcov-229e.fasta \\
        -O {REFERENCE}/hcov-229e.dict
else
    echo "Reference dictionary {REFERENCE}/hcov-229e.dict ${{green}}already present${{reset}}"
fi
...
if [[ ! -f {REFERENCE}/ref_genome_autosomal.interval_list ]]; then
    ...
    egrep '(NC_002645.1)\\s' {REFERENCE}/hcov-229e.fasta.fai |
        awk '{{print $1"\\t1\\t"$2"\\t+\\t"$1}}' |
        cat {REFERENCE}/hcov-229e.dict - >{REFERENCE}/ref_genome_autosomal.interval_list
else
...
if [[ ! -f {VCF} ]]; then
    ...
    bcftools mpileup \\
        ...
        --threads 4 \\
        --output-type u \\
        --fasta-ref {REFERENCE}/hcov-229e.fasta \\
        {BAM} 2>/dev/null | \\
    bcftools call \\
...
def runPipeline(script: TextIOWrapper, options: OptionsDict, prefix: str):
    ...
    script.write(
        """
if [[ ! -f {REFERENCE}/hcov-229e.flat.fasta ]]; then
    tail -n +2 {REFERENCE}/hcov-229e.fasta | 
        grep -v '^>' | 
        tr -d '\\n' | 
        sed 's/\\(.\\)/\\1 /g' | 
        tr ' ' '\\n' > {REFERENCE}/hcov-229e.flat.fasta
else
...
if [[ ! -d {STATS}/{SAMPLE}_bcfstats ]]; then
    bcftools stats --fasta-ref {REFERENCE}/hcov-229e.fasta {PIPELINE}/{SAMPLE}.vcf > {STATS}/{SAMPLE}.chk
...
if [[ ! -f {STATS}/{SAMPLE}.samtools.coverage ]]; then
    samtools coverage -d 0 \\
        --reference {REFERENCE}/hcov-229e.fasta \\
