...
def updateDictionary(script: TextIOWrapper, options: OptionsDict):
    ...
    script.write(
        """
if [[ ! -f {REFERENCE}/hcov-oc43.dict ]]; then
    java -jar {BIN}/picard.jar CreateSequenceDictionary \\
        -R {REFERENCE}/hcov-oc43.fasta \\
        -O {REFERENCE}/hcov-oc43.dict
else
...
if [[ ! -f {VCF} ]]; then
    ...
    bcftools mpileup \\
        ...
        --threads 4 \\
        --output-type u \\
        --fasta-ref {REFERENCE}/hcov-oc43.fasta \\
        {BAM} 2>/dev/null | \\
    bcftools call \\
...
def runPipeline(script: TextIOWrapper, options: OptionsDict, prefix: str):
    ...
    script.write(
        """
if [[ ! -f {REFERENCE}/hcov-oc43.flat.fasta ]]; then
    tail -n +2 {REFERENCE}/hcov-oc43.fasta | 
        grep -v '^>' | 
        tr -d '\\n' | 
        sed 's/\\(.\\)/\\1 /g' | 
        tr ' ' '\\n' > {REFERENCE}/hcov-oc43.flat.fasta
else
...
if [[ ! -d {STATS}/{SAMPLE}_bcfstats ]]; then
    bcftools stats --fasta-ref {REFERENCE}/hcov-oc43.fasta {PIPELINE}/{SAMPLE}.vcf > {STATS}/{SAMPLE}.chk
...
if [[ ! -f {STATS}/{SAMPLE}.samtools.coverage ]]; then
    samtools coverage -d 0 \\
        --reference {REFERENCE}/hcov-oc43.fasta \\
